...
def evaluatev0(respth='./pretrained', dspth='./data', backbone='CatNetSmall', scale=0.75, use_boundary_2=False, use_boundary_4=False, use_boundary_8=False, use_boundary_16=False, use_conv_last=False):
    ...
    net.load_state_dict(torch.load(respth).state_dict())
    net.eval()
    # inputs = torch.randn(1, 3, 512, 1024).float()
    # traced_model = torch.jit.trace(net, torch.randn(1, 3, 512, 1024).float(), strict=False)
    # traced_model.save('stdc150_trace.pt')
    # torch.save(net.state_dict(), "stdc250-nozip.pt",_use_new_zipfile_serialization=False)
...
if __name__ == "__main__":
    ...
    evaluatev0('stdc150_line_trace.pt', dspth='./data', backbone='STDCNet813', scale=0.5, 
