...
def prediect(respth='./pretrained', dspth='./data', backbone='CatNetSmall', scale=0.75, use_boundary_2=False,
    ...
    with torch.no_grad():
        for i, (imgs, img_pth) in enumerate(dl):
            ...
            new_hw = [2048, 2048] # 输入尺寸大效果更好,比例要和训练图片比例一致效果好
            new_imgs = F.interpolate(imgs, new_hw, mode='bilinear', align_corners=True)
            new_imgs = new_imgs.cuda()
            logits = net(new_imgs)[0] # [0]是经过 FFM - 8x 的结果
            ...
            skeleton_out_path = skeleton_out_path[:-4]+'_res.jpg'
