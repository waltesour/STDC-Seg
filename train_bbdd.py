...
def parse_args():
    ...
    parse.add_argument(
            '--n_classes',
            dest = 'n_classes',# 分割类别数目=实际数目+1(背景)
            type = int,
            default = 3,
            )
    parse.add_argument(
    ...
    parse.add_argument(
            '--use_conv_last',
            dest = 'use_conv_last', ##是否对STDC主干网络中stage5的特征图上多进行一次ConvX(conv-BN-Relu)
            type = str2bool,
            default = False,
            )
    parse.add_argument(
            '--use_boundary_2', #是否返回STDC主干网络中stage1的特征图(尺寸是原图1/2)
            dest = 'use_boundary_2',
            type = str2bool,
            default = False,
            )
    parse.add_argument(
            '--use_boundary_4', #是否返回STDC主干网络中stage2的特征图(尺寸是原图1/4)
            dest = 'use_boundary_4',
            type = str2bool,
            default = False,
            )
    parse.add_argument(
            '--use_boundary_8', #是否返回STDC主干网络中stage3的特征图(尺寸是原图1/8)
            dest = 'use_boundary_8',
            type = str2bool,
            default = True,
            )
    parse.add_argument(
            '--use_boundary_16', #是否返回STDC主干网络中stage4的特征图(尺寸是原图1/16)
...
def train():
    ...
    mode = args.mode
    cropsize = [int(args.cropsize.split(',')[0]),int(args.cropsize.split(',')[1])]
    ...
    criteria_p = OhemCELoss(thresh=score_thres, n_min=n_min, ignore_lb=ignore_idx)# 在线难例挖掘交叉熵损失
    criteria_16 = OhemCELoss(thresh=score_thres, n_min=n_min, ignore_lb=ignore_idx)
    criteria_32 = OhemCELoss(thresh=score_thres, n_min=n_min, ignore_lb=ignore_idx)
    boundary_loss_func = DetailAggregateLoss() # STDC网络架构中的Detail Loss
    ## optimizer
    maxmIOU50 = 0.
    maxmIOU75 = 0.
    maxmIOU100 = 0.
    ...
    for it in range(max_iter):
        try:
            ...
            diter = iter(dl)
            im, lb = next(diter) #lb是标签图像
        im = im.cuda()
        lb = lb.cuda()
        # print(im.shape)
        # print(lb[0][0][1024, 1216])
        H, W = im.size()[2:]
        ...
        lossp = criteria_p(out, lb) #Seg Loss
        ...
        if use_boundary_2: 
            ...
            boundery_bce_loss2,  boundery_dice_loss2 = boundary_loss_func(detail2, lb) #Detail Loss
        ...
        if (it+1)%save_iter_sep==0:# and it != 0:
            ...
            with torch.no_grad():
                ...
                mIOU100 = single_scale3(net, dlval, n_classes)

            save_pth = osp.join(save_pth_path, 'model_iter{}_mIOU50_{}_mIOU75_{}_mIOU100_{}.pth'
            .format(it+1, str(round(mIOU50,4)), str(round(mIOU75,4)), str(round(mIOU100,4))))
